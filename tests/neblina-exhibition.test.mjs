...
test("two new stills and two edited clips resolve into published exhibition", async () => {
  ...
  for (const slot of [...stills, ...motions]) {
    const bytes = readFileSync(new URL(`../public${slot.src}`, import.meta.url));
    assert.equal(createHash("sha256").update(bytes).digest("hex"), slot.provenance.sha256);
  }
  assert.ok(motions.every((slot) => /edited.*still/i.test(slot.description)));
  const gallery = JSON.parse(readFileSync(new URL("../public/gallery/index.json", import.meta.url), "utf8"));
  assert.equal(gallery.images.filter((entry) => entry.tags.includes("neblina")).length, 7);
  for (const slot of [...stills, ...motions]) {
    ...
    assert.ok(readFileSync(new URL(`../public${entry.thumb}`, import.meta.url)).length > 0);
  }
});
